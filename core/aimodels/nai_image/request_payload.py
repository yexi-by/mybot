...
from .huashichuan import 预设画师串_二次元_正面, 预设画师串_二次元_负面
...
def get_payload(
    ...
    if v4_prompt_char_captions:
        ...
        for char_caption in v4_prompt_char_captions:
            # 将 CharCaption Pydantic 模型转换为字典
            char_caption_dict = char_caption.model_dump()
            payload["parameters"]["v4_prompt"]["caption"]["char_captions"].append(char_caption_dict)
            
            # 将 Center 对象列表转换为字典列表
            centers_dicts = [center.model_dump() for center in char_caption.centers]
            negative_char_data = {
                "char_caption": "",  # 负面提示词为空字符串
                "centers": centers_dicts  # 使用转换后的字典列表
            }
            payload["parameters"]["v4_negative_prompt"]["caption"]["char_captions"].append(negative_char_data)
            
            # 将 Center 对象转换为字典
            center_dict = char_caption.centers[0].model_dump()
            char_prompt_data = {
                "prompt": char_caption.char_caption,
                "uc": "",
                "center": center_dict,  # 使用转换后的字典
                "enabled": True
            }
